...
interface JwtPayload {
  sub: string;       // Email (JWT standard subject claim from Spring Boot)
  id: number;        // Profile ID (teacher/student/user id from Spring Boot)
...
export const authMiddleware = (
  ...
  try {
    ...
    // Spring Boot uses 'sub' for email, 'id' for profile ID, 'role' for role
    ...
    // Map Spring Boot JWT fields to our internal user structure
    req.user = {
      userId: decoded.id,
      email: decoded.sub,
      role: decoded.role,
